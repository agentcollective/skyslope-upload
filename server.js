...
function normalizeName(firstName, lastName, fullName) {
  ...
  if (full) {
    ...
    if (resolvedFirst.toLowerCase() === "jeff") resolvedFirst = "Geoff";
    return {
  ...
  return { firstName: "Unknown", lastName: "Client", fullName: "Unknown Client" };
}
...
function normalizePhone(rawPhone) {
  ...
  if (digits.length === 7) digits = `${DEFAULT_PHONE_AREA_CODE}${digits}`;
  if (digits.length > 10) digits = digits.slice(0, 10);
  if (digits.length < 10) digits = (`${DEFAULT_PHONE_AREA_CODE}${digits}`).padEnd(10, "0").slice(0, 10);
  return digits;
}
...
function normalizeAttachments(input) {
  ...
  if (!Array.isArray(attachments)) throw new Error("Attachments payload must be an array or JSON string.");
  ...
  const normalized = attachments
    ...
    .map((a, idx) => {
      ...
      if (fileName.length > 150) fileName = fileName.slice(0, 146).trim() + ".pdf";
      return { url, fileName, mimeType };
    })
    .filter(Boolean);

  if (normalized.length === 0) throw new Error("No valid attachments after normalization.");
  return normalized;
}
...
function buildSkySlopeAuthHeader() {
  ...
  const hmac = crypto.createHmac("sha256", SKYSLOPE_ACCESS_SECRET).update(input).digest("base64");
  return { timestamp, authorization: `SS ${SKYSLOPE_ACCESS_KEY}:${hmac}` };
}

async function getSkySlopeSession() {
  const now = Date.now();
  if (sessionCache.token && now < sessionCache.expiresAt - 60000) return sessionCache.token;
  ...
  if (!session) throw new Error(`SkySlope login failed. Unexpected response: ${JSON.stringify(response.data)}`);
...
// CORRECTED: no property object. Pre-Contract buyer file needs only office, agent, checklist.
...
async function createPreContractSale(payloadOverrides = {}) {
  ...
  if (!saleGuid) throw new Error(`Could not determine saleGuid from create response: ${JSON.stringify(result)}`);
  return { payload, result, saleGuid };
}
...
async function uploadDocumentToSale(saleGuid, attachment) {
  ...
  const payload = { fileName: attachment.fileName, base64Content };
...
app.get("/auth-test", async (req, res) => {
  try {
    ...
    return okResponse(res, { ok: false, step: "auth-test", error: summarizeAxiosError(error) });
  }
});

// DEBUG: pulls the real Source IDs, Status IDs, and other form options from SkySlope.
// Open in browser: /debug/sale-form  then search the JSON for "Source" and "Status".
app.get("/debug/sale-form", async (req, res) => {
  try {
    const result = await skySlopeRequest("get", "/api/files/saleForm");
    return okResponse(res, { ok: true, form: result });
  } catch (error) {
    return okResponse(res, { ok: false, step: "sale-form", error: summarizeAxiosError(error) });
  }
});

app.get("/debug/create-payload", (req, res) => {
  return okResponse(res, { ok: true, payload: buildCreateSalePayload() });
});

app.post("/debug/test-create-sale", async (req, res) => {
  try {
    const result = await createPreContractSale(req.body || {});
    return okResponse(res, { ok: true, step: "create-sale", saleGuid: result.saleGuid, payload: result.payload, result: result.result });
  } catch (error) {
    return okResponse(res, { ok: false, step: "create-sale", payloadTried: buildCreateSalePayload(req.body || {}), error: summarizeAxiosError(error) });
  }
});
...
app.post("/api/skyslope-upload", async (req, res) => {
  ...
  try {
    ...
    if (sharedSecret !== WEBHOOK_SHARED_SECRET) {
      return okResponse(res, { ok: false, step: "auth", error: "Unauthorized" });
    }

    const clientName = normalizeName(req.body.clientFirstName, req.body.clientLastName, req.body.clientFullName);
    ...
    const attachments = normalizeAttachments(req.body.attachments || req.body.attachmentsJson);

    debug.push({ step: "input-normalized", buyer, agent, attachmentCount: attachments.length });

    let createResult;
    try {
      createResult = await createPreContractSale();
      saleGuid = createResult.saleGuid;
      debug.push({ step: "sale-created", saleGuid, payload: createResult.payload });
    } catch (createError) {
      return okResponse(res, {
        ok: false,
        step: "create-sale",
    ...
    try {
      ...
      debug.push({ step: "buyer-added", result: buyerResult });
    } catch (buyerError) {
      debug.push({ step: "buyer-add-failed", error: summarizeAxiosError(buyerError) });
    }
    ...
    for (const attachment of attachments) {
      try {
        ...
        uploaded.push({ fileName: attachment.fileName, uploaded: true, result });
      } catch (uploadError) {
        uploaded.push({ fileName: attachment.fileName, uploaded: false, error: summarizeAxiosError(uploadError) });
      }
    }
    ...
    try {
      ...
      debug.push({ step: "sale-detail-failed", error: summarizeAxiosError(detailError) });
    }
    ...
    return okResponse(res, {
      ...
      uploaded,
      summaryText: `SkySlope upload completed for ${buyer.fullName}. Created pre-contract buyer file. Uploaded ${uploadedSuccessCount} document(s). Failed uploads: ${uploadedFailCount}.`,
      debug
    });
  } catch (error) {
    return okResponse(res, { ok: false, step: "unhandled", saleGuid, error: summarizeAxiosError(error), debug });
  }
});
